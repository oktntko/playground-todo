...
この問題の趣旨は、入力値をどこまで考慮できたか？という点です。

意図通りに厄年をだせた、だけど適当な入力値を与えるとエラーになってしまう。
じゃあ、こういう入力値に対してはこうしよう、なーんてことを考えられていれば💯
...
入力値があるときは、次のポイントを確認しましょう。

- どんな入力値がありうるか？を列挙すること
  - 重要なのは、意図通りの入力値と、そうでない入力値を切り分けること
- 意図通りでない入力値をどう打ち返してあげるのが適切か？
  - 重要なのは、誤った入力値を入力した人に解決策を示すこと
